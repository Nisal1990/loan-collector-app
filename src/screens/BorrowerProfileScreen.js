  ...
  return (
    <Card className="mb-4" noPadding={true}>
      <TouchableOpacity 
        ...
        activeOpacity={0.7}
      >
        <Text className="font-bold text-primary dark:text-white text-base">{title}</Text>
        {isOpen ? <ChevronUp color="#94A3B8" size={20} /> : <ChevronDown color="#94A3B8" size={20} />}
      </TouchableOpacity>
      {isOpen && (
        <View className="px-4 pb-4 pt-1 border-t border-gray-100 dark:border-slate-700">
          {children}
        </View>
      )}
    </Card>
...
const DataRow = ({ label, value, isBold = false, isDanger = false, isSuccess = false }) => (
  <View className="flex-row justify-between py-2 border-b border-gray-50 dark:border-slate-700/50">
    <Text className="text-gray-500 dark:text-gray-400 w-[40%]">{label}</Text>
    <Text className={`w-[60%] text-right ${isBold ? 'font-bold' : ''} ${isDanger ? 'text-danger' : ''} ${isSuccess ? 'text-accent' : 'text-primary dark:text-white'}`}>
      {value || '-'}
    </Text>
  </View>
);
...
export default function BorrowerProfileScreen({ route, navigation }) {
  ...
  // If no borrowerId is passed (e.g. from tab directly, though it shouldn't normally happen), default to first borrower
  ...
  const handleWhatsApp = () => {
    // In a real app we'd use react-native-communications or Linking
  ...
  return (
    <SafeAreaView className="flex-1 bg-background dark:bg-slate-900">
      {/* Header */}
      <View className="flex-row items-center px-4 py-3 bg-white dark:bg-slate-800 border-b border-gray-100 dark:border-slate-700">
        <TouchableOpacity onPress={() => navigation.goBack()} className="p-2 -ml-2">
          <ChevronLeft color={iconColor} size={24} />
        </TouchableOpacity>
        <Text className="flex-1 text-center text-lg font-bold text-primary dark:text-white mr-6">Profile Details</Text>
      </View>
      ...
      <ScrollView className="flex-1 px-4 pt-4 pb-24" showsVerticalScrollIndicator={false}>
        ...
        <View className="items-center mb-6 mt-2">
          {borrower.photoUrl ? (
            ...
            <View className="w-24 h-24 rounded-full bg-blue-100 dark:bg-blue-900/50 items-center justify-center mb-3">
              <Text className="text-xl font-bold text-primary dark:text-blue-100">{borrower.initials}</Text>
            </View>
          )}
          <Text className="text-xl font-bold text-primary dark:text-white">{borrower.fullName}</Text>
          <Text className="text-gray-500 dark:text-gray-400 mb-2">A/C: {borrower.loanAccountNo}</Text>
          <Badge 
        ...
        <AccordionSection title="Section E - Payment History">
          <View className="mb-4 mt-2">
            <View className="flex-row border-b-2 border-gray-200 dark:border-slate-700 pb-2 mb-2">
              <Text className="flex-[2] font-bold text-gray-700 dark:text-gray-300 text-xs">Date</Text>
              <Text className="flex-[2] font-bold text-gray-700 dark:text-gray-300 text-xs text-right">Pay Amt</Text>
              <Text className="flex-[2] font-bold text-gray-700 dark:text-gray-300 text-xs text-right">Capital</Text>
              <Text className="flex-[2] font-bold text-gray-700 dark:text-gray-300 text-xs text-right">Interest</Text>
            </View>
            
            {borrower.payments.length > 0 ? (
              borrower.payments.map((pay) => (
                <View key={pay.id} className="flex-row border-b border-gray-100 dark:border-slate-700/50 py-2">
                  <Text className="flex-[2] text-xs text-primary dark:text-white">{pay.date}</Text>
                  <Text className="flex-[2] text-xs text-primary dark:text-white font-semibold text-right">{pay.amount}</Text>
                  <Text className="flex-[2] text-xs text-gray-500 dark:text-gray-400 text-right">{pay.capital}</Text>
                  <Text className="flex-[2] text-xs text-gray-500 dark:text-gray-400 text-right">{pay.interest}</Text>
                </View>
              ))
            ) : (
              <Text className="text-gray-400 dark:text-gray-500 text-center py-4 text-sm">No payments recorded</Text>
            )}
          </View>
          
          <View className="bg-gray-50 dark:bg-slate-700/50 p-3 rounded-xl border border-gray-100 dark:border-slate-700 mt-2">
            <Text className="font-bold text-primary dark:text-white mb-2 border-b border-gray-200 dark:border-slate-600 pb-2">Summary</Text>
            ...
            <View className="flex-row justify-between py-2 border-b border-gray-50 dark:border-slate-700/50">
              <Text className="text-gray-500 dark:text-gray-400 w-[40%]">Arrears/Over Pay</Text>
              <View className="w-[60%] items-end">
                {borrower.status === 'Completed' || borrower.outstanding === 0 ? (
                  <Badge label="Loan Paid" status="success" />
                ) : (
                  <Text className="text-danger font-bold">{formatCurrency(borrower.arrearsAmount)}</Text>
                )}
              </View>
            </View>
          ...
          {borrower.penalties && borrower.penalties.length > 0 ? (
            <View>
              {borrower.penalties.map(pen => (
                <View key={pen.id} className="flex-row justify-between items-center py-2 border-b border-gray-100 dark:border-slate-700/50">
                  <View>
                    <Text className="text-primary dark:text-white font-bold">{pen.date}</Text>
                    <Text className="text-gray-500 dark:text-gray-400 text-xs">{pen.reason}</Text>
                  </View>
                  <View className="items-end">
                    <Text className="text-danger font-bold">{formatCurrency(pen.amount)}</Text>
                    <Text className={`text-xs ${pen.status === 'Paid' ? 'text-accent' : 'text-danger'}`}>{pen.status}</Text>
                  </View>
                </View>
              ))}
              <View className="mt-3 pt-3 border-t border-gray-200 dark:border-slate-700 flex-row justify-between">
                <Text className="font-bold text-gray-700 dark:text-gray-300">Total Penalties</Text>
                <Text className="font-bold text-danger">{formatCurrency(borrower.penaltyOutstanding)}</Text>
              </View>
            </View>
          ) : (
            <Text className="text-gray-400 dark:text-gray-500 py-3">No penalties applied to this account.</Text>
          )}
        </AccordionSection>
      ...
      <View className="absolute bottom-0 left-0 right-0 bg-white dark:bg-slate-900 border-t border-gray-200 dark:border-slate-800 p-4 pb-6 flex-row justify-between">
        <Button 
          title="Collect" 
        ...
        <TouchableOpacity 
          ...
          className="w-14 h-14 border border-primary dark:border-slate-500 rounded-xl items-center justify-center bg-transparent active:opacity-80"
        >
          <MessageCircle color={iconColor} size={24} />
        </TouchableOpacity>
        <TouchableOpacity 
          onPress={handleSMS}
          className="w-14 h-14 border border-primary dark:border-slate-500 rounded-xl items-center justify-center bg-transparent ml-2 active:opacity-80"
        >
